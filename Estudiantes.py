...
def validar_nombre(nombre,matriz_estudiantes):
    ...
    if len(matriz_estudiantes)==0:
        while not nombre.replace(" ", "").isalpha():#verifica que solo haya letras y elimina el espacio para poder analizar el nombnre
        ...
        while salida!=0:
            ...
            for i in range(len(matriz_estudiantes)):
                ...
                if nombre == matriz_estudiantes[i][1]:
                    ...
                    valido=False
                elif not nombre.replace(" ", "").isalpha():#verifico que solo haya letras en el nombre y reemplaza los espacios por nada asi el codigo lo entiende
                    nombre=input("el nombre no es valido, hay numeros dentro del nombre, porfavor ingrese uno nuevo: ")
                    valido=False
